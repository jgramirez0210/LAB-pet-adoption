...
const filterCat = document.querySelector("#btn-cat");

btnFilterCat.addEventListener('click',() => {
  const filteredPets = filter('cat');
  filterCards(filteredPets)
})

const filterDino = (type) => {
  return pets.filter(pet => pet.type === type)
}
const filterDino = document.querySelector("#btn-dino");

filterDino.addEventListener('click',() => {
  const filteredPets = filter('dino');
  filterCards(filteredPets)
})
